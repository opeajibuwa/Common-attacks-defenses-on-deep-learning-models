"""This script trains adversarial models and validates them on the test data."""
...
parser.add_argument('--eps', default=0.3, help="epsilon for PGD attack")
...
def train_and_validate(model, model_name, train_dataloader, val_dataloader, loss_fn, optimizer, epochs, device, accuracy):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for X, y in train_dataloader:
            ...
            X_adv = PGD(model, X, y, 7, step_size = 2/255, eps = float(args.eps))
            
            model.train()
            
            if model_name == 'lenet_cifar10' or model_name == 'resnet_mnist':
                y_pred, probas = model(X_adv)
            else: y_pred = model(X_adv)
    ...
    MODEL_PATH = Path("adv_saved_models")
